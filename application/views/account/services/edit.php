    ...
    <div class="widthfix">
        ...
        <div id="service-fields" class="right">
            ...
            <div class="content-box right" id="service-addfield">
                ...
                <div class="box-content">
                    <form autocomplete="off" class="form-validated" id="add-field-form" accept-charset="utf-8" method="post" action="<?php echo site_url('account/edit_service/' . $service->getServiceId()) ?>">
                        <table width="100%" cellspacing="0" cellpadding="0" class="boxed-table">
                            ...
                            <tr class="">
                                <td>
                                    <p class="clearfix left">
                                        ...
                                        <select name="fieldType" id="fieldType" class="text required tiptip">
                            ...
                            <tr class="even">
                                <td>
                                    <p class="clearfix">
                                        <label>Field Value(s)<br><span style="color:e47074;">*</span> per row for select1</label>
...
//            CKEDITOR.instances.addTextText.setData('');
        ...
        $("form").submit(function (event) {
            $("#editFieldValue").val($("#editFieldValue").val().trim());

            var isEditForm = $(this).attr('id') === 'edit-field-form'; // Check if it's the edit form
            var isAddForm = $(this).attr('id') === 'add-field-form'; // Check if it's the edit form            
            var fieldType, fieldValue;
            // If it's the edit form, use editFieldType and editFieldValue
            if (isEditForm) {
                fieldType = $("#editFieldType").val();
               // fieldValue = $("#editFieldValue").val().trim();
                fieldValue = cleanupLineBreaks($("#editFieldValue").val().trim()); // Clean line breaks and trim whitespace
                $("#editFieldValue").val(fieldValue);

                // Remove the error class for the edit case
                $("#editFieldValue").removeClass("error");
                // Check if fieldType is 'select' and fieldValue is empty for the edit form
                if ((fieldType === "select" || fieldType === "number") && fieldValue === "") {
                   // alert("Field Value(s) cannot be empty when Field Type is 'Dropdown' or 'Number' for Edit case.");
                    $("#editFieldValue").addClass("error"); // Add error class to textarea for edit
                    event.preventDefault(); // Prevent form submission
                }
            } 
            if(isAddForm){ // Otherwise, it's the add form
                fieldType = $("#fieldType").val();
                //fieldValue = $("#fieldValue").val().trim();
                fieldValue = cleanupLineBreaks($("#fieldValue").val().trim()); // Clean line breaks and trim whitespace
                $("#fieldValue").val(fieldValue); // Update the textarea with the cleaned value

                // Remove the error class for the add case
                $("#fieldValue").removeClass("error");
                // Check if fieldType is 'select' and fieldValue is empty for the add form
                if ((fieldType === "select" || fieldType === "number") && fieldValue === "") {
                    //alert("Field Value(s) cannot be empty when Field Type is 'Dropdown' or 'Number' for Add case.");
                    $("#fieldValue").addClass("error"); // Add error class to textarea for add
                    event.preventDefault(); // Prevent form submission
                }
            }
        });
    ...
    function cleanupLineBreaks(value) {
        console.log("Value",value.replace(/(\r\n|\n|\r){2,}/g, '\n').trim());
    // Replace multiple line breaks (2 or more) with a single line break
        ...
        <table width="100%" cellspacing="0" cellpadding="0" class="boxed-table">
            ...
            <tr class="">
                <td>
                    <p class="clearfix left">
                        <label>Field Type</label>
                        <select name="editFieldType" id="editFieldType" class="tiptip">
                            <option value="text">Text</option>
                            <option value="select">Dropdown</option>
                            <option value="textarea">Text Area</option>
                        </select>
            ...
            <tr class="even">
                <td>
                    <p class="clearfix">
                        <label>Default Value(s)<br><span style="color:e47074;">*</span>1 per row for select2</label>
