...
#otpResend.ui-state-default {
            cursor: pointer;
            opacity: 1;
        }

        #otpResend.ui-state-disabled {
            cursor: not-allowed;
            opacity: 0.5;
        }
div.warning{
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#sendPasswordReset").click(function () {
            //Hide opt box reset the setting or reset div inside dialog box
            $("#sendPasswordEmailHelp").dialog('close');  // Ensure it's closed first
            // Reset form fields or content inside the dialog (optional)
    ...
    function clearTimeInterval(){
                clearInterval(expireInterval);  // Stop the expire timer
                $("#expire_timer_count").hide();  // Hide the timer div
                $("#expire_timer").text('00:00');  // Reset timer display to 00:00
                $("#msg_error").hide();  // Hide any error messages
    }

  

    $(document).on('click', '.ui-dialog-titlebar-close', function () {
    clearTimeInterval();  // Call the function when the "X" icon is clicked
    });
    ...
    $("#otpResend").hide();
    $("#otp-label, #otp-input, #msg_success, #msg_error, #timer_count,#expire_timer_count").hide();
    $(".Otp_box").hide();
...
var expireInterval;  // Store the timer reference globally
function startExpireTimer(duration) {
    clearInterval(expireInterval);  // Clear any previously running timer
    var timer = duration || timerDuration;  // Set the timer duration
    var minutes, seconds;

    $("#expire_timer_count").show();  // Show the timer div when starting the timer
    ...
    expireInterval = setInterval(function () {
        ...
        minutes = minutes < 10 ? "0" + minutes : minutes;
        seconds = seconds < 10 ? "0" + seconds : seconds;
        $("#expire_timer").text(minutes + ":" + seconds);

        if (--timer < 0) {
            clearInterval(expireInterval);  // Stop the timer when it reaches zero
            $("#expire_timer_count").hide();  // Hide the timer div
            $("#msg_error").show();
            $("#msg_error").text("Verification Code expired");
        }
    }, 1000);  // Update every second
}
//expire timer close
  ...
  success: function (data) {
      if (data.auth) {
        ...
        $(".send_varification_code").hide();
          ...
          $("#msg_success").show();
          $("#expire_timer_count").show();
          startExpireTimer(data.expire_time);
...
<div id="sendPasswordEmailHelp" title="Verification Code">
          <?php 
 ...
 <div id="otp-verification-dialog" title="2way Authentication" style="display:none;">
    <?php if($user->getCellPhone() != "") { ?>
        <p>A verification code has been sent to : <b><?php echo $user->getCellPhone() ?></b></p>
