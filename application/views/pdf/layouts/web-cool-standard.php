                ...
                <form id="clientSignatureForm" onsubmit="return false;" class="g-3 needs-validation-checklist" novalidate>
                    ...
                    <input type="hidden" name="proposal_id" id="proposal_id" value="<?php echo $proposal->getProposalId(); ?>">
                        ...
                        <div class="col-md-6">
                            ...
                            <?php $addressParts = [];
                                                if (isset($proposalChecklistData)) {
                                                    if ($proposalChecklistData->getBillingAddress()) {
                                                        $addressParts[] = trim($proposalChecklistData->getBillingAddress());
                                                    }
                                                } else {
                                                    if ($proposal->getClient()->getBillingAddress()) {
                                                        $addressParts[] = trim($proposal->getClient()->getBillingAddress());
                                                    }
                                                    if ($proposal->getClient()->getBillingCity()) {
                                                        $addressParts[] = trim($proposal->getClient()->getBillingCity());
                                                    }
                                                    if ($proposal->getClient()->getBillingState()) {
                                                        $addressParts[] = trim($proposal->getClient()->getBillingState());
                                                    }
                                                    if ($proposal->getClient()->getBillingZip()) {
                                                        $addressParts[] = trim($proposal->getClient()->getBillingZip());
                                                    }
                                                }
                                                    $addressComplete =  implode(', ', $addressParts);                                                  
                                                   ?>

                            <div class="row">
                            <div class="col-md-12 mb-3">
                                    <label for="billing_address" class="form-label">Billing Address</label>
                                     <textarea class="form-control" name="billing_address" id="billing_address" placeholder="Please Billing Address"><?php echo trim($addressComplete);?></textarea>
                            ...
                            <div class="row">
                            <div class="col-md-12 mb-3">
                               <p class="clearfix"> <input type="checkbox" name="same_address" id="same_address" style="width: 14px; margin: 3px 0;"><span style="margin-left:7px;">Legal Address same as Billing Address</span></p>
                        ...
                        <div class="row">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="hidden" id="invoicing_portal" value="<?php  if(isset($proposalChecklistData)) {echo $proposalChecklistData->getInvoicingPortal();} ?>">
  ...
  <script>
    ...
    $(document).on("click", ".proposal-slider-close, .new-popup-close-btn", function() {
      ...
      function checkAddresses() {
            let billingAddress = $('#billing_address').val().trim();
            let legalAddress = $('#legal_address').val().trim();
            
            if (billingAddress !== '' && billingAddress === legalAddress) {
                // If they are the same, check the checkbox
        ...
        // Run the address check on page load
        checkAddresses();
      $('#same_address').on('change', function () {
            if ($(this).is(':checked')) {
                // If the checkbox is checked, copy the Billing Address into the Legal Address
                let billingAddress = $('#billing_address').val();
                $('#legal_address').val(billingAddress);
            } else {
                // If the checkbox is unchecked, clear the Legal Address
                $('#legal_address').val('');
            }
        });
